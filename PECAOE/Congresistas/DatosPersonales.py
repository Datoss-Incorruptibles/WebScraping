...
#   print(outFile.read())
    ...
    for partidoPolitico in docString:
        ...
        TXJEE = partidoPolitico["TXJEE"]
        IDESTLISTA = partidoPolitico["IDESTLISTA"]        # 4 retirada , #1 inscrita 

        OBJPARTIDO = {
        "TXORGPOLITICA":TXORGPOLITICA, 
        "IDEXPEDIENTE":IDEXPEDIENTE,
        "TXJEE":TXJEE,
        "IDESTLISTA":IDESTLISTA
        }
...
for PARTIDO_POLITICO in PARTIDOSPOLITICOS:
  try:  
    ...
    for i in range(len(listaDeCantidatos)): 
        ...
        XRESCANDIDATOS.append(CANDIDATOABC)

    PARTIDO_POLITICO["CANDIDATOS"] = XRESCANDIDATOS
  except:
    print("CRASH OBTIENDO LA LISTA DE CANDIDATOS DE UNA CIUDAD")
    PARTIDO_POLITICO["CANDIDATOS"] = []
  ...
  for CANDIDATO in PARTIDO_POLITICO["CANDIDATOS"]:
    try:
      if CANDIDATO["HOJA_VIDA_ENCRIPTADA"] == "VACIO":
        ...
        # CandidatoSinHV = Guardar tmb la info de los sin CV ?
      else:
